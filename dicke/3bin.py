...
def parse_arguments():
    ...
    # System Configuration (Bin 3 - Extra Neutrinos)
    group_bin3 = parser.add_argument_group('Bin 3 Configuration (Extra Electron Neutrinos)')
    group_bin3.add_argument('--e2', type=int, default=1, help="number of electron neutrinos in bin 3 (default 1)")
    group_bin3.add_argument('--energy3', type=float, default=1.2, help="energy of bin 3")

    # Output
    parser.add_argument('--savename', type=str, default='3bin', help="base name of the saved figure")
...
def print_simulation_info(args, n1, n2, n3):
    """
    ...
    print(f"Bin 3: {n3} Electron Neutrinos (Extra) @ Energy={args.energy3}")
...
def solve_mean_field(args, l_table, constants):
    ...
    Returns:
        tuple: (mft_p_e_bin1, mft_p_e_bin2, mft_p_e_bin3)
    """
    ...
    omega2 = -constants['dmsq'] / (2.0 * args.energy2) # Anti-neutrino negative freq
    omega3 = constants['dmsq'] / (2.0 * args.energy3)

    # 2. Build parameter arrays
    mft_omega = np.array([omega1] * n1 + [omega2] * n2 + [omega3] * n3)
    
    # Initial flavors: "e" for neutrinos, "ebar" for anti-neutrinos
    mft_initial_flavours = ["e"] * n1 + ["ebar"] * n2 + ["e"] * n3

    # 3. Interaction Matrix
    if n_total > 0:
        j_matrix = (args.j / n_total) * np.ones((n_total, n_total))
    else:
        j_matrix = np.zeros((0, 0))
    ...
    p_e_raw = 0.5 * (1 + mft_sol[:, 2, :]) # Convert Pz to Probability
    
    # Average within each bin
    p_e_bin1 = np.mean(p_e_raw[:n1, :], axis=0) if n1 > 0 else np.zeros(len(l_table))
    p_e_bin2 = np.mean(p_e_raw[n1:n1+n2, :], axis=0) if n2 > 0 else np.zeros(len(l_table))
    p_e_bin3 = np.mean(p_e_raw[n1+n2:, :], axis=0) if n3 > 0 else np.zeros(len(l_table))

    print("[MFT] Evaluation complete.")
    return p_e_bin1, p_e_bin2, p_e_bin3
...
def solve_dicke_heisenberg(args, l_table, constants):
    ...
    Always assumes 3 bins structure for simplicity.
    WARNING: Will crash (division by zero in observables) if any bin has 0 particles!
    """
    ...
    # Bin 1: Nu (Up), Bin 2: AntiNu (Down), Bin 3: Nu (Up)
    n_up_list = [n1, n2, n3]
    n_down_list = [0, 0, 0]
    
    omega_list = [omega1, omega2, omega3]
    is_antineutrino_flags = [False, True, False]
    
    # 1. Build Initial State
    psi0, S_list = dc.multi_bin_initial_state(n_up_list, n_down_list)
    ...
    H, (Jx_list, Jy_list, Jz_list), S_list, dims = dc.build_multi_bin_hamiltonian(
        ...
        is_antineutrino=is_antineutrino_flags,
    )
...
def plot_mft_vs_heisenberg(t_stream, heis_p_e, l_table, mft_p_e, args, S_list, constants):
    ...
    mft_p_bin1, mft_p_bin2, mft_p_bin3 = mft_p_e
    ...
    ax1.plot(t_stream, heis_p_e[:,2], label='Bin 3 (Heisenberg)', 
             color="tab:green", ls="--", marker="x", markevery=mk_step, ms=ms_size, alpha=0.8)
    ...
    ax1.plot(l_table, mft_p_bin3, label=f'Bin 3 (MFT)', 
             color="green", ls="-.", lw=2)
...
def main():
    ...
    print_simulation_info(args, n1, n2, n3)
