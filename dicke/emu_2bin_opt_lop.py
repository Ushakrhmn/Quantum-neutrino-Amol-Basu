"""
Two-bin neutrino oscillation simulation using Dicke collective states with LinearOperators.

This version uses the memory-patched implementation with LinearOperator mode enabled by default,
which provides:
- Reduced memory footprint (no explicit Hamiltonian matrix storage)
- Compatible with expm_multiply for time evolution
- Numerically accurate with complex128 dtype

The LinearOperator mode is controlled by the DICKE_USE_LINEAR_OPERATOR environment variable
(default=1). The implementation has been validated to match the baseline dense solver.
"""
# --- Memory monitoring instrumentation (adapted from Lilith's instrumentation) ---
...
# --- End memory monitoring instrumentation ---

import mft
import dicke_linearop as dc  # optimized API with LinearOperator support
import numpy as np
from datetime import datetime
...
parser.add_argument('--mem-interval', type=float, default=2.0, help='Memory monitoring interval in seconds')
args = parser.parse_args()

# --- Instrumentation init ---
_log = setup_logger()
_log.info("args: %s", vars(args))
_wstop, _wthread = start_mem_watcher(_log, interval_sec=args.mem_interval)
atexit.register(lambda: (_wstop.set(), log_mem(_log, "atexit")))
log_mem(_log, "startup")
...
_log.info("Hamiltonian built: H type=%s, dims=%s", type(H).__name__, dims)
log_mem(_log, "after build_H")

print("Evolving with LinearOperators ...")
log_mem(_log, "before evolve")

# Use regular evolve_times (works with LinearOperators)
states = dc.evolve_times(H, psi0, l_table, dtype=dc.DEFAULT_DTYPE)
print("Evolution complete.")
log_array(_log, "states", states)
log_mem(_log, "after evolve")

print("Computing observables ...")
_, dc_p_e = dc.bin_observables(states, Jz_list, S_list)
print("Dicke solution evaluated.")
log_array(_log, "dc_p_e", dc_p_e)
log_mem(_log, "after observables")
...
plt.grid(alpha=0.3)
plt.tight_layout()
_log.info("saving figure..."); log_mem(_log, "before savefig")
...
_log.info("saving figure..."); log_mem(_log, "before savefig_2")
plt.savefig(output_filename)
print(f"Plot saved to {output_filename}")
log_mem(_log, "before show_2"); plt.show()