import mft
import dicke_collective_sparse_opt_proj as dc  # optimized streaming API
import numpy as np
...
parser.add_argument('--coherent-only', choices=['none','projection'], default='none', help='If set to projection, project to product SU(2)-coherent state after each dt (keeps single-particle entropy ≈ 0; approaches MFT).')
...
if args.coherent_only == 'projection':
    stream = dc.evolve_times_stream_projected(H, psi0, l_table, Jx_list, Jy_list, Jz_list, S_list, normalize=args.normalize)
    t_stream, Jz_t, dc_p_e_full = dc.observables_from_stream(stream, Jz_list, S_list)
    dc_p_e = dc_p_e_full
else:
...
plt.plot(t_stream, dc_p_e[:,0], label=f'Bin 1 (N={int(2*S_list[0])}, E={args.energy1:.2f})' + (' [proj]' if args.coherent_only=="projection" else ''), color="blue")
plt.plot(t_stream, dc_p_e[:,1], label=f'Bin 2 (N={int(2*S_list[1])}, E={args.energy2:.2f})' + (' [proj]' if args.coherent_only=="projection" else ''), color="red")
plt.xlabel('baseline')
plt.ylabel('Pe')
plt.legend()
plt.grid(alpha=0.3)
...
ax1.plot(t_stream, dc_p_e[:,0], label=f'Bin 1 (N={int(2*S_list[0])}, E={args.energy1:.2f})' + (' [proj]' if args.coherent_only=="projection" else ''), color="blue")
ax1.plot(t_stream, dc_p_e[:,1], label=f'Bin 2 (N={int(2*S_list[1])}, E={args.energy2:.2f})' + (' [proj]' if args.coherent_only=="projection" else ''), color="red")
...
def _compute_entropy_stream_projected(H, psi0, t_grid, Jx_list, Jy_list, Jz_list, S_list, *, normalize=True):
    """Stream with projection and return times + single-particle entropies per bin (nats)."""
    stream = dc.evolve_times_stream_projected(H, psi0, t_grid, Jx_list, Jy_list, Jz_list, S_list, normalize=normalize)
    K = len(S_list)
    N_list = [int(2*S) for S in S_list]
    t_acc = []
    ...
    for (t, psi) in stream:
        t_acc.append(t)
        # Entropy per bin from Bloch vector length r = 2|⟨J⃗⟩|/N (nats)
        # After projection, r≈1 so S≈0, but we compute anyway.
        for a in range(K):
            Jx = float(np.vdot(psi, (Jx_list[a] @ psi)))
            Jy = float(np.vdot(psi, (Jy_list[a] @ psi)))
            Jz = float(np.vdot(psi, (Jz_list[a] @ psi)))
            N = float(N_list[a])
            r = float(2.0 * np.sqrt(Jx*Jx + Jy*Jy + Jz*Jz) / max(N, 1.0))
            r = float(np.clip(r, 0.0, 1.0))
            lam_p = 0.5*(1.0 + r)
            # binary entropy in nats
            S_acc[a].append( -lam_p*np.log(lam_p + 1e-300) - (1-lam_p)*np.log(1-lam_p + 1e-300) )
    t_acc = np.asarray(t_acc, float)
    S_bins = [np.asarray(S_acc[a], float) for a in range(K)]
    return t_acc, S_bins
...
try:
    ...
    if args.coherent_only == 'projection':
        t_S, S_bins = _compute_entropy_stream_projected(
