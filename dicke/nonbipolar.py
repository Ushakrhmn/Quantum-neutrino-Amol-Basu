"""
Non-bipolar neutrino oscillation simulation.

Compares Many-Body (Dicke) vs Mean Field Theory (MFT) for a two-bin system
with different energies:
  - Bin 1: ν_e at energy E1
  - Bin 2: ν̄_e at energy E2
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(description="Non-bipolar neutrino oscillation simulation.")
    ...
    # Output
    parser.add_argument('--savename', type=str, default='nonbipolar',
...
def setup_physics_constants():
    ...
    return {
        'theta': np.pi/2 - 0.2,  # Mixing angle
...
def print_simulation_info(args, constants):
    ...
    print("=" * 50)
    print("NON-BIPOLAR SIMULATION CONFIGURATION")
    print("=" * 50)
    print(f"  Bin 1: {n1} ν_e @ E={args.energy1}  |  Bin 2: {n2} ν̄_e @ E={args.energy2}")
    print(f"  ω₁ = {omega1:.4f}  |  ω₂ = {omega2:.4f}")
    print(f"  J = {args.j}  |  θ = {constants['theta']:.4f}  |  Δm² = {constants['dmsq']}")
...
def solve_mean_field(args, l_table, constants):
    ...
    # Frequencies: antineutrinos have negative omega
    omega1 = constants['dmsq'] / (2 * args.energy1)
    omega2 = -constants['dmsq'] / (2 * args.energy2)
...
def solve_dicke_model(args, l_table, constants):
    ...
    # Different energies → different omega magnitudes
    omega1 = constants['dmsq'] / (2 * args.energy1)
    omega2 = constants['dmsq'] / (2 * args.energy2)
    
    # Initial state: all ν_e in bin 1, all ν̄_e in bin 2
    psi0, S_list = dc_linop.multi_bin_initial_state([n1, n2], [0, 0])
    ...
    # Build Hamiltonian (is_antineutrino handles sign flip)
...
def plot_comparison(t_dicke, p_dicke, t_mft, p_mft, args, constants):
    ...
    fig, (ax_main, ax_res) = plt.subplots(2, 1, figsize=(12, 10), height_ratios=[2, 1], sharex=True)
    ...
    ax_main.plot(t_mft, mft_bin1, color=COLOR_NU, ls="--", lw=2, label="ν_e (MFT)")
    ax_main.plot(t_mft, mft_bin2, color=COLOR_NUBAR, ls="--", lw=2, label="ν̄_e (MFT)")
    
    ax_main.set_ylabel("Survival Probability")
    ax_main.set_title("Non-Bipolar Flavor Evolution: Many-Body vs Mean Field")
    ax_main.legend(loc="best")
    ax_main.grid(True, alpha=0.3)
    
    # Parameter info box
    info = f"θ = {constants['theta']:.3f}\nΔm² = {constants['dmsq']}\nJ = {args.j}"
    ...
    ax_res.text(0.02, 0.95, stats, transform=ax_res.transAxes,
    ...
    filename = f"{args.savename}_{timestamp}_n{n_total}.png"
