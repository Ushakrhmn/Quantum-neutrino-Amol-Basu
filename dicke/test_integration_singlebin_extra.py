...
# -*- coding: utf-8 -*-
"""
Integration test: baseline (dense) single-bin Dicke vs matrix-free extra implementation.
...
The baseline uses exact diagonalization (dense); the extra uses matrix-free streaming RK4.
...
dc_opt  = _import_module("dicke_collective_sparse_opt_extra", "dicke_collective_sparse_opt_extra.py")


# ---- testing core -----------------------------------------------------------------------------

def build_case(n1, n2, mu, theta, *, l=16.0, s=512, omega=1.0, normalize=False, method="rk4"):
    """
    Build and run both implementations for a single bin, returning (t, Pee_base, Pee_opt).
    """
    N = int(n1 + n2)
    if N <= 0:
    ...
    # ---- matrix-free (extra) ----
    H_o, (Jx_o, Jy_o, Jz_o), S_o, d_o = dc_opt.make_linear_operator_single_bin(
        N=N, omega=omega, theta_v=theta, mu=mu, subtract_muS2=False
    )
    # Build identical Dicke |S,m> with m=(n1-n2)/2
    m = (n1 - n2) / 2.0
    psi0_o = dc_opt.dicke_basis_vector(S_o, m)

    stream = dc_opt.evolve_times_stream(H_o, psi0_o, t_grid, normalize=normalize, gc_between=True, method=method)
    t_stream, Jz_t_o, Pee_o = dc_opt.observables_from_stream(stream, Jz_o, S_o)
    ...
    # Pee_b shape (T,1); Pee_o shape (T,) -> tile to match for comparison
    Pee_b_flat = Pee_b[:, 0]
    Pee_o_flat = Pee_o

    return t_grid, Pee_b_flat, Pee_o_flat
...
def agrees_within(Pee_b, Pee_o, atol=2e-5, rtol=2e-5):
...
def run_suite(N_list=(2, 10, 50), mu_list=(0.0, 0.05, 0.5), theta_list=(math.pi/4, math.pi/2 - 0.2),
              l=16.0, s=512, omega=1.0, normalize=False, method="rk4", atol=2e-5, rtol=2e-5, max_cases=None):
    """
    ...
    for (n1, n2, mu, theta) in cases:
        try:
            t, Pee_b, Pee_o = build_case(n1, n2, mu, theta, l=l, s=s, omega=omega, normalize=normalize, method=method)
            max_abs, rms = max_errors(Pee_b, Pee_o)
...
def main(argv=None):
    import argparse
    p = argparse.ArgumentParser(description="Integration test: dense vs matrix-free single-bin (P_ee agreement).")
    ...
    p.add_argument("--method", type=str, default="rk4", choices=["rk4", "expm"], help="Streaming integrator.")
