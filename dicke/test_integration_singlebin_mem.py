...
# -*- coding: utf-8 -*-
"""
Integration test: baseline (dense) single-bin Dicke vs sparse memory-optimized implementation.
...
The baseline uses exact diagonalization (dense); the mem version uses sparse matrices + streaming.
...
dc_mem  = _import_module("dicke_collective_sparse_opt_mem", "dicke_collective_sparse_opt_mem.py")


# ---- testing core -----------------------------------------------------------------------------

def build_case(n1, n2, mu, theta, *, l=16.0, s=512, omega=1.0, chunk=64, normalize=False, dtype=None):
    """
    Build and run both implementations for a single bin, returning (t, Pee_base, Pee_mem).
    """
    N = int(n1 + n2)
    if N <= 0:
    ...
    # ---- sparse memory-optimized (streaming) ----
    psi0_m, S_m = dc_mem.single_bin_initial_state(n1, n2, dtype=dtype)
    H_m, (Jx_m, Jy_m, Jz_m), S_list_m, dims_m = dc_mem.build_single_bin_hamiltonian(
        N=N, omega=omega, theta_v=theta, mu=mu, dtype=dtype
    )
    
    # Use streaming evolution
    stream = dc_mem.evolve_times_stream(H_m, psi0_m, t_grid, chunk=chunk, normalize=normalize, dtype=dtype, gc_every=1)
    t_stream, Jz_t_m, Pee_m = dc_mem.observables_from_stream(stream, [Jz_m], S_list_m)
    ...
    # Pee_b shape (T,1); Pee_m shape (T,1) -> flatten for comparison
    Pee_b_flat = Pee_b[:, 0]
    Pee_m_flat = Pee_m[:, 0]

    return t_grid, Pee_b_flat, Pee_m_flat


def max_errors(Pee_b, Pee_m):
...
def run_suite(N_list=(2, 10, 50), mu_list=(0.0, 0.05, 0.5), theta_list=(math.pi/4, math.pi/2 - 0.2),
    ...
    for (n1, n2, mu, theta) in cases:
        try:
            t, Pee_b, Pee_m = build_case(n1, n2, mu, theta, l=l, s=s, omega=omega, 
                                         chunk=chunk, normalize=normalize, dtype=dtype)
            max_abs, rms = max_errors(Pee_b, Pee_m)
            ok = agrees_within(Pee_b, Pee_m, atol=atol, rtol=rtol)
...
def main(argv=None):
    import argparse
    p = argparse.ArgumentParser(description="Integration test: dense vs sparse memory-optimized single-bin (P_ee agreement).")
    ...
    p.add_argument("--chunk", type=int, default=64, help="Streaming chunk size.")
    p.add_argument("--normalize", action="store_true", help="L2-normalize |psi| at each step (streaming).")
    p.add_argument("--dtype", type=str, default="complex64", choices=["complex64", "complex128"], help="Numeric dtype.")
    ...
    ok = run_suite(N_list=args.N, mu_list=args.mu, theta_list=args.theta,
                   ...
                   chunk=args.chunk, normalize=args.normalize, dtype=dtype,
