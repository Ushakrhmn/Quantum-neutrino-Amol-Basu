...
# -*- coding: utf-8 -*-
"""
Integration test: baseline sparse vs patched sparse two-bin Dicke implementation.
...
The baseline uses explicit sparse matrices (dicke_collective_sparse_opt_mem.py); 
the patched version uses LinearOperators by default (dicke_collective_sparse_opt_mem_patched.py).
...
dc_base = _import_module("dicke_collective_sparse_opt_mem", "dicke_collective_sparse_opt_mem.py")
dc_patched = _import_module("dicke_collective_sparse_opt_mem_patched", "dicke_collective_sparse_opt_mem_patched.py")
...
def build_case(e1, m1, e2, m2, mu, theta, *, l=16.0, s=512, omega=1.0, dtype=None):
    ...
    # ---- baseline sparse (explicit sparse matrices) ----
    psi0_b, S_list_b = dc_base.multi_bin_initial_state([e1, e2], [m1, m2], dtype=dtype)
    ...
    states_b = dc_base.evolve_times(H_b, psi0_b, t_grid, dtype=dtype)
    _, Pee_b = dc_base.bin_observables(states_b, Jz_list_b, S_list_b)

    # ---- patched sparse (LinearOperator by default) ----
...
def main(argv=None):
    import argparse
    p = argparse.ArgumentParser(description="Integration test: baseline sparse vs patched sparse two-bin (P_ee agreement).")
